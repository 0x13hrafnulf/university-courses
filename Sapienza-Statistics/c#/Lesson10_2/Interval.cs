...
namespace Lesson10_2
{
    ...
    public class IntervalList
    {
        ...
        public IntervalList(int count, string name, int index, int points, Type type)
        {
            m_count = count;
            m_name = name;
            m_index = index;
        ...
        {
            if (m_type == typeof(Int32)) populateInt(min, max);
            else populateDouble(min, max);
        }
        public void populateInt(double min, double max)
        {
        ...
        public void populateDouble(double min, double max)
        {
            double size = (max - min);
            size = Math.Round(size / m_count, 2);
            double start = min;
            for (int i = 0; i < m_count; ++i)
            {
                Interval interv = new Interval(Math.Round(start, 2), size);
                m_intervals.Add(interv);
                start += size;
            }
        }
    ...
    public class bivariate_distribution
    {
        ...
        public void compute_relative_frequencies()
        {
            for (int i = 0; i < m_x_intervals.m_count + 1; ++i)
            {
                for (int j = 0; j < m_y_intervals.m_count + 1; ++j)
                {
                    m_relative_frequency[i, j] = Math.Round((double) m_frequency[i, j] / (double) m_frequency[m_x_intervals.m_count, m_y_intervals.m_count], 2);
                }
            }          
        }
