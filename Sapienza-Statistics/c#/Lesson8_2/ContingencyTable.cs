...
namespace Lesson8_2
{
    public class ContingencyTable : Viewport
    {
        ...
        public override void draw(Graphics G, Dataset dt)
        {
            ...
            int m_grid_x = dt.m_summary_data[1].m_intervals + 1;
            int m_grid_y = dt.m_summary_data[0].m_intervals + 1;
            ...
            G.DrawString(dt.m_summary_data[0].m_name, font, fontBrush, m_table.X, m_table.Y + m_table.Height + m_pad / 3);
            G.DrawString(dt.m_summary_data[1].m_name, font, fontBrush, m_table.X + m_pad / 2 + grid_w, m_table.Y - m_pad / 2);

            for (int i = 1; i <= dt.m_summary_data[0].m_intervals; ++i)
            {
                G.DrawString(dt.m_intervals[0].m_intervals[i - 1].ToString(), font, fontBrush, m_table.X + m_pad / 5, m_table.Y + m_pad / 2 + grid_h * i);
            }
            for (int i = 1; i <= dt.m_summary_data[1].m_intervals; ++i)
            {
            ...
            for (int i = 0; i <= dt.m_summary_data[0].m_intervals; ++i)
            {
                ...
                {
                    G.DrawString(dt.m_distributions[0].m_frequency[i, j].ToString(), font, fontBrush, m_table.X + m_pad / 2 + grid_w * (j + 1), m_table.Y + m_pad / 2 + grid_h * (i + 1));
                }
            }
