...
int main(int argc, char **argv)
{
    ...
    if(file_system.node->type != KRFFS_Root_Node)
    {
        fprintf(
            stderr,
            "Not a root node at the beginning of the file!\n",
            path
        );
    ...
    struct krffs_node* prev_node = cur_node;
    do
    {
        ...
        if(!(krffs_is_node_in_file_system(&file_system, cur_node)))
        {
            fprintf(
            stderr,
            "Found a link leading outside the file system space!\n",
            path
            );
        ...
        if(cur_node->magic != KRFFS_File_System_Magic)
        {
            fprintf(
            stderr,
            "Found a node with an invalid signature!\n",
            path
            );
        ...
        if(cur_node->type != KRFFS_Root_Node &&
           cur_node->type != KRFFS_Free_Node &&
           cur_node->type != KRFFS_Reserved_Node)
        {
            fprintf(
            stderr,
            "Found a node of an unknown type!\n",
            path
            );
