...
namespace svm
{
    ...
    void CPU::Step()
    {
		...
		//1. Getting page index and physical address offset from MMU for data	
		...
		if(frame == Memory::INVALID_PAGE) {
			int temp = registers.a;//saving value of register
			registers.a = page_offset_i.first;//placing index of the page into the register
			_pic.isr_4();//calling the page fault handler
			registers.a = temp;//restoring register's value
		} else {
			registers.a = _memory.ram[frame + page_offset_i.second];//calculation of the phys.address using GetPageIndex....
			registers.ip += 2;
			++instruction;//incrementing instruction pointer
		}
