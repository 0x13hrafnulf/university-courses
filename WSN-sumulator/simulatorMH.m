...
%%%%%Window%%%%%
main_window = figure('Name', 'SimulatorMH', 'Units', 'Normalized', 'Position', [0, 0, 0.6, 0.9], 'Visible', 'off', 'MenuBar', 'none', 'Resize', 'on');
datacursormode on;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
    ...
    function calculate_energy(round)
                ...
                for j=1:n_nodes+1
                    if(j < n_nodes+1)
                        if(nodes(j,5) == 0)
                            ...
                            if(i ~= j)
                                if(d0 > Dist(i,j))
                                    Etx = (msg + msg_header_size)*(Eelec) + (msg + msg_header_size)*Efs*Dist(i,j)^2;
                                elseif(d0 < Dist(i,j))
                                    Etx = (msg + msg_header_size)*(Eelec) + (msg + msg_header_size)*Emp*Dist(i,j)^4;
                                end
                                Erx = (msg + msg_header_size)*(Eelec);
                                Etotal = Etx + Erx;
                                Weights(i, j) = Etotal;           
                            end
                        end
        ...
        Weights(n_nodes+1,:) = transpose(Weights(:,n_nodes+1));
        path_graph = digraph(Weights);
        for i = 1:n_nodes     
            if(nodes(i,5) == 0) 
                continue;
            else
                ...
                for j=1:size(path,2)-1
                    ...
                    if(id == i)
                        ...
                        node_energy_per_round(id, round) = node_energy_per_round(id, round) + Etotal;
                        ...
                        Erx = (msg_s - msg + msg_header_size)*(Eelec) + EDA * (msg_s + msg_header_size);
                        Etotal = Etx+Erx;
                        total_energy_per_round(round) = total_energy_per_round(round) + Etotal;
                        node_energy_per_round(id, round) = node_energy_per_round(id, round) + Etotal;
                        nodes(id, 6) = nodes(id, 6) - Etotal;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%DRAWING%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    function draw_path(path)
         ...
         for j=1:size(path,2)-1
            ...
            next_id = path(j + 1);
            if(next_id ~= n_nodes+1)   
                handler_lines = [handler_lines, plot([nodes(id,1), nodes(next_id,1)], [nodes(id,2), nodes(next_id,2)],'-.b', 'LineWidth', 2)];
            else
                handler_lines = [handler_lines, plot([nodes(id,1), bs_x], [nodes(id,2), bs_y],'-.r', 'LineWidth', 2)];
            end
         end   
         hold(ax1, 'off');
    end
